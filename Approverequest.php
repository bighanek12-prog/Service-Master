...
?>
  ...
  while($row = $result->fetch_assoc()){
        ...
        echo '<div class="card-header text-primary">';
        echo 'Request ID : '. $row['request_id'];
         echo '</div>';
        echo '<div class="card-body">';
       echo '<h5 class="card-title text-info">Request Info : ' . $row['request_info'] . '</h5>';
      ...
      echo '<form action="" method="POST"> <input type="hidden" name="id" value='. $row["request_id"] .'>
         <input type="submit" class="btn btn-inverse-success " name="view" value="View">
         <input type="submit" class="btn btn-inverse-danger " name="close" value="Close"></form>';
       echo '</div>' ;
      echo '</div>' ;
     echo'</div>';
  }
